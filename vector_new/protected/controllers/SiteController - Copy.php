...
class SiteController extends Controller
{
    ...
    public function actionSearching()
	{
		...
		if(isset($_POST['selection'])){
			//throw new CHttpException (401, "dentro");
			...
			if($_POST['selection']=='Issue'){
					...
					if(preg_match($reg_ex, $_POST['word'])){
						...
						if($projects==''){  //when no project is selected
							$sql = "SELECT	 result.id, 
									...
									result.colonne, 
									result.ligne, 
									LOCATE('$condition', value) as position
								FROM result 
								JOIN issue ON result.issueId=  issue.id 
								JOIN traveler ON  issue.travelerId = traveler.id
								JOIN components ON traveler.componentId =  components.id 
								WHERE result.value LIKE '%$condition%' ORDER BY result.id";
					
						} else $sql = "SELECT	 result.id, result.elementId, result.value, result.issueId, result.fileId, result.createTime, result.userId, result.colonne, result.ligne     
								FROM result 
								JOIN issue ON result.issueId=  issue.id 
								JOIN traveler ON  issue.travelerId = traveler.id
								JOIN components ON traveler.componentId =  components.id 
								WHERE result.value LIKE '%$condition%' and components.projectId = $projects ORDER BY result.id";
				...
				} else throw new CHttpException(403, "hola");
			} 
					...
					if(preg_match($reg_ex, $_POST['word'])){
						...
						if($projects==''){
							$sql = "SELECT	 element.id, 
										...
										step.name stepName
										FROM    element 
										JOIN step ON element.stepId=  step.id 
										JOIN traveler 	ON step.travelerId=  traveler.id
										JOIN components 	ON traveler.componentId=  components.id
										WHERE element.label like '%$condition%'  ORDER BY traveler.id";
							
						} else $sql = "SELECT	 element.id, 
										element.label, 
										element.stepId, 
										components.projectId,
										traveler.name, 
										step.name stepName
									FROM    element 
									JOIN step ON stepId=  step.id 
									JOIN traveler 	ON step.travelerId=  traveler.id
									JOIN components 	ON traveler.componentId=  components.id
									WHERE label like '%$condition%' and components.projectId = $projects  ORDER BY traveler.id";
				...
				if(isset($_POST['word'])){
					if(preg_match($reg_ex, $_POST['word'])){
						...
						if($projects==''){
							$sql = "SELECT		file.id fileId,
									file.stepId,
									file.fileSelected,
									traveler.id,
									traveler.name
								FROM    file 
									JOIN step ON file.stepId=  step.id 
									JOIN traveler 	ON step.travelerId=  traveler.id
									JOIN components 	ON traveler.componentId=  components.id
									WHERE file.fileSelected like '%$condition%' and file.stepId is NOT null
								UNION	
								SELECT	file.id fileId,
									file.stepId,
									file.fileSelected,
									issue.id,
									traveler.name
								FROM    file 
									JOIN result ON  file.id =  result.value
									JOIN issue 	ON result.issueId=  issue.id
									JOIN traveler 	ON traveler.id=  issue.travelerId
									JOIN equipment 	ON issue.equipmentId=  equipment.id
									JOIN components	ON equipment.componentId=  components.id
									WHERE file.fileSelected like '%$condition%' and file.stepId is null";
							
						} else $sql = "SELECT		file.id fileId,
									file.stepId,
									file.fileSelected,
									traveler.id,
									traveler.name
								FROM    file 
									...
									JOIN traveler 	ON step.travelerId=  traveler.id
									JOIN components 	ON traveler.componentId=  components.id
									WHERE file.fileSelected like '%$condition%' and file.stepId is NOT null and components.projectId=$projects
								UNION	
								SELECT	file.id fileId,
									file.stepId,
									file.fileSelected,
								...
								FROM    file 
									...
									JOIN equipment 	ON issue.equipmentId=  equipment.id
									JOIN components	ON equipment.componentId=  components.id
									WHERE file.fileSelected like '%$condition%' and file.stepId is null and components.projectId=$projects";
