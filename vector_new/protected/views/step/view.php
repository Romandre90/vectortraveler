...
if(Yii::app()->user->getState('role') > 1){  //USERS ABOVE 1 HAVE LINKS BECAUSE THEY CAN ACCESS TO PROJECTS, COMPONENTS AND 
	if ($issue) {
		$title=Yii::t('default','Project >> Component >> Equipment >> Issued traveler > Step');
		Yii::app()->clientScript->registerScript('breadcrumbs', "document.getElementById('breadcrumbs').title='$title'; ");
		$issueId = $issue->id;
		...
		if ($model->parentId):
			$this->breadcrumbs = array(
				$model->traveler->project->name => array('project/view/','id'=> $model->traveler->project->id),//project
				$model->traveler->component->name=> array('components/view','id'=>$model->traveler->componentId),//component
				$issue->equipment->identifier => array('equipment/view', 'id' => $issue->equipmentId),//equipment
				$issue->traveler->name => array('issue/view', 'id' => $issue->id),//traveler
				Yii::t('default', 'Step') . ' ' . $model->parent->position . ".0" => array('view', 'id' => $model->parentId, 'issueId' => $issueId),//steps
		...
		$issueId = null;
		$status = false;
		$title=Yii::t('default','Project >> Component >> Traveler template > Step');
		Yii::app()->clientScript->registerScript('breadcrumbs', "document.getElementById('breadcrumbs').title='$title'; ");
		if ($model->parentId):
			$this->breadcrumbs = array(
				$model->traveler->project->name => array('project/view/','id'=> $model->traveler->project->id),
				$model->traveler->component->name=> array('components/view','id'=>$model->traveler->componentId),
				$model->traveler->name => array('traveler/view', 'id' => $model->travelerId),
	...
	} else {// USERS BELOW 2 don't have links!
	if ($issue) {
		$title=Yii::t('default','Project >> Component >> Equipment >> Issued traveler > Step');
