...
foreach ($steps as $step) {
    if (!$step->parent)
        if($step->haveDiscrepancy($issueId))
            echo CHtml::image('../../images/warning.png');
...
foreach ($steps as $step): $si = 1;
    ...
    <div class="view <?php echo $class ?>">
        <h2>
            ...
            if (!$step->parent)
                echo "<a href='#' id='$step->id'><img src='../../images/top.gif'></a> ";
            if($discrepancy)
                echo CHtml::link(CHtml::image('/images/warning.png'),array('nonconformity/view','id'=>$discrepancy->id));
            if($save){
