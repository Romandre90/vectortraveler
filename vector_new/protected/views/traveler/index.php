...
$this->breadcrumbs = array(
    Yii::t('default', 'Traveler templates'),
);
*/
$this->menu = array(
...
<h1><?php echo Yii::t('default', 'Traveler templates') ?></h1>
<ul id='treeview' class='filetree'>
    ...
    foreach ($query as $q):
        ...
        if ($projectId != $pid):;
            ...
            $archive = "";
            ?>
            <li class="p<?php echo $pid ?>" <?php echo $style ?>><?php echo "<span class='project' title='Project: Identifier + Name'>" . $pname . "</span>"; ?>
                <ul>
                <?php endif; ?>
                <?php if ($componentId != $cid): ?>
                    ...
                    ?>
                    <li class="c<?php echo $cid ?>" <?php echo $style ?>><?php echo "<span class='component' title='Component: Identifier + Name'>" . $cname . "</span>"; ?>
                        <ul>
                        <?php endif; ?>
                        <?php if ($traveler->getNode($workId)): ?>
                            <?php echo "$archive$ul" ?>
                            <li><?php echo "<span class='templateSet' title='Traveler Name'>" . trim(CHtml::encode($tname)) . "</span>"; ?>
                                <ul>
                                <?php endif; ?>
                                <?php
                                if ($traveler->status == 2 && $status != $traveler->status) {
                                    echo "<li class='archive' $styleA><span class='archive'>" . Yii::t('default', 'Deprecated versions') . "</span><ul>";
                                }
                                ?>
                                <li><span class='template'><?php echo CHtml::link("(v" . $traveler->revision . ") " . $traveler->statusText, array('view', 'id' => $traveler->id),array('onclick'=>'mostra_loading_screen()')) ?></span></li>
                                <?php
                                $projectId = $pid;
                                $componentId = $cid;
